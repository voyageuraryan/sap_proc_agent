...
def parse_po_filter(raw: str | None) -> str:
    ...
    if not shape_check:
        raise ODataError(
            code="UNSUPPORTED_FILTER", message="The Query String shape doesn't match", status=400
        )
...
def reject_unsupported_options(params: Mapping[str, str], allowed: set[str]) -> None:
    for key in params:
        if key.startswith("$") and key not in allowed:
            raise ODataError(
                code="UNSUPPORTED_OPTION", message=f"{key} parameter is not allowed", status=400
            )
